import { Dashboard, dashboardSchema } from "@/apis/dashboards/types";
import ColumnList from "@/components/columns/ColumnList";
import axiosServerHelper from "@/utils/network/axiosServerHelper";
import { safeResponse } from "@/utils/network/safeResponse";
import { Metadata } from "next";

export async function generateMetadata({
  params,
}: {
  params: Promise<{ id: string }>;
}): Promise<Metadata> {
  const resolvedParams = await params;
  const id = resolvedParams.id;
  try {
    const response = await Promise.race([
      axiosServerHelper<Dashboard>(`/dashboards/${id}`),
      new Promise<never>((_, reject) => {
        setTimeout(() => reject(new Error("Timeout")), 1500);
      }),
    ]);
    const dashboardDetail = safeResponse(response.data, dashboardSchema);
    return {
      title: `Taskify - ${dashboardDetail.title}`,
      description: "Taskify 대시보드에서 커뮤니티 일정을 한눈에 관리해 보세요.",
      openGraph: {
        title: `Taskify - ${dashboardDetail.title}`,
        description:
          "Taskify 대시보드에서 커뮤니티 일정을 한눈에 관리해 보세요.",
        url: `https://taskify-namgyeon.vercel.app/dashboard/${id}`,
        type: "website",
        images: [
          {
            url: "https://taskify-namgyeon.vercel.app/meta.png",
            width: 1200,
            height: 630,
            alt: "Taskify 대시보드 페이지 이미지",
          },
        ],
      },
    };
  } catch (error) {
    if (error instanceof Error && error.message === "Timeout") {
      return {
        title: "Taskify - 대시보드 로딩 실패",
      };
    }
    throw error;
  }
}

const DashboardDetailPage = () => {
  return <ColumnList />;
};

export default DashboardDetailPage;
